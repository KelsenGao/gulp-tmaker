...
module.exports = (function name(params) {
		...
		FormatField.forEach(function (element) {
			if (conf[element] && conf[element].length > 0) {
				...
				conf[element].split('|').forEach(function (item) {
					rvalue += Tagbase.Format[element + 'Format'][item];
				}, this);
				...
				if (element == 'site') {
					...
					conf[element] = rvalue ? conf[element] : rvalue;
				}
	...
	function enpHeadBuild (TmakerHead) {
		...
		if (core.options.isPreview) {
			...
			// 如果是 SingleNode
			if (_bot == 'SingleNode' && _merged.szinfo != '单个栏目') {
	...
	function getEnpField(key){
		...
		if(core.options.isPreview){
			...
			_temp = Tagbase.Preview.CommonField[key] || Tagbase[curTmake.head._private.bot].field[key] || 'null';
			_temp = _temp.split('|');
			if(Salt.hasOwnProperty(_temp[0])){
				return Salt[_temp[0]](_temp[1]);
			}else{
				return Tagbase.Preview.CommonField[key] || '{#无效字段}';
			}
			
		}else{
			
			return Tagbase.CommonField[key] || Tagbase[curTmake.head._private.bot].field[key] || '9';
		}
	}
	 ...
	 */
	function fieldRander (fieldValue) {
		var _str = fieldValue[1],
			dir = core.options.isPreview ? Tagbase.Preview[fieldValue[0].slice(1)] : Tagbase.Format[fieldValue[0].slice(1)];
		
		if(_str && _str.length>0){
			return _str.replace(/[a-zA-Z]/g,function(letter){
				return dir[letter];
			});
		} else {
			return dir.default;
	...
	function enpTplBuild (tpl) {
		...
		tpl = tpl.replace(/\{(\$[\S\s]*?)\}/g,function(all, letter){
			...
			_field = letter.split('|');
			_temp = getEnpField(_field[0].replace(/\s/,''));
			
			// 如果有配置项
			
			if(_temp.search(/s%/) >= 0){
				_temp = _temp.replace('s%', fieldRander(_field));
			}
			return _temp;
		});
